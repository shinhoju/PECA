...
def rectified_padding(img_lq, img_gt, img_gd, gt_size):
    h, w, _ = img_lq.shape
    ...
    if h_pad == 0 and w_pad == 0:
        return img_lq, img_gt, img_gd

    img_lq = cv2.copyMakeBorder(img_lq, 0, h_pad, 0, w_pad, cv2.BORDER_REFLECT)
    img_gt = cv2.copyMakeBorder(img_gt, 0, h_pad, 0, w_pad, cv2.BORDER_REFLECT)
    img_gd = cv2.copyMakeBorder(img_gd, 0, h_pad, 0, w_pad, cv2.BORDER_REFLECT)
    ...
    if img_gd.ndim == 2:
        img_gd = np.expand_dims(img_gd, axis=2)
    return img_lq, img_gt, img_gd
