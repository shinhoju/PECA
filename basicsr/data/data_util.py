...
def rectified_paired_paths_from_folder(folders, keys, filename_tmpl):
    """Generate paired paths from folders.

    Args:
        folders (list[str]): A list of folder path. The order of list should
            be [input_folder, gt_folder, gd_folder].
        keys (list[str]): A list of keys identifying folders. The order should
            be in consistent with folders, e.g., ['lq', 'gt', 'gd'].
        filename_tmpl (str): Template for each filename. Note that the
            template excludes the file extension. Usually the filename_tmpl is
            for files in the input folder.
    ...
    assert len(folders) == 3, (
        'The len of folders should be 2 with [input_folder, gt_folder, gd_folder]. '
        f'But got {len(folders)}')
    assert len(keys) == 3, (
        'The len of keys should be 2 with [input_key, gt_key, guide_key]. '
        f'But got {len(keys)}')
    ...
    assert len(input_paths) == len(gt_paths) == len(guide_paths), (
        f'{input_key} and {gt_key}, {guide_key} datasets have different number of images: '
        f'{len(input_paths)}, {len(gt_paths)}, {len(guide_paths)}.')
    paths = []
