...
def rectified_paired_random_crop(img_gts, img_lqs, img_gds, lq_patch_size, scale, gt_path):
    """Paired random crop.
    ...
    Returns:
        list[ndarray] | ndarray: GT images and LQ images, GD images. If returned results
    ...
    h_gd, w_gd, _ = img_gds[0].shape
    gt_patch_size = int(lq_patch_size * scale)
    gd_patch_size = int(lq_patch_size * scale)
    ...
    # crop corresponding gd patch
    top_gd, left_gd = int(top * scale), int(left * scale)
    img_gds = [
        v[top_gd:top_gd + gd_patch_size, left_gd:left_gd + gd_patch_size, ...]
        for v in img_gds
    ]
